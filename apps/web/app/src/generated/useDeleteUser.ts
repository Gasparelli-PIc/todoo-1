...
export const deleteUserMutationKey = () => [{ url: '/usuarios/:id' }] as const
...
export async function deleteUser(id: DeleteUserPathParams["id"], config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<DeleteUserMutationResponse, ResponseErrorConfig<DeleteUser404>, unknown>({ method : "DELETE", url : `/usuarios/${id}`, ... requestConfig })  
  return res.data
}
