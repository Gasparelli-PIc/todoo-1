...
export const updateUserMutationKey = () => [{ url: '/usuarios/:id' }] as const
...
export async function updateUser(id: UpdateUserPathParams["id"], data?: UpdateUserMutationRequest, config: Partial<RequestConfig<UpdateUserMutationRequest>> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<UpdateUserMutationResponse, ResponseErrorConfig<UpdateUser404>, UpdateUserMutationRequest>({ method : "PUT", url : `/usuarios/${id}`, data : requestData, ... requestConfig })  
  return res.data
}
